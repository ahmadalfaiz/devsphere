...
/**
 * Valid five-letter target words.
 *
 * A random word from this list is selected for every
 * new game.
 *
 * This list can later be moved into a dedicated data
 * file when the Wordle word collection becomes larger.
 */
const TARGET_WORDS = [
...
/**
 * All currently accepted guesses.
 *
 * For this first version, target words are also valid
 * guesses. This keeps the game completely self-contained.
 *
 * The list can later be expanded with a much larger
 * accepted-guess dictionary without changing the game
 * logic.
 */
const VALID_GUESSES = new Set(TARGET_WORDS);
...
/**
 * Creates an empty status board.
 */
const createEmptyStatusBoard = () =>
  Array.from(
    { length: MAX_ATTEMPTS },
    () =>
      Array(WORD_LENGTH).fill(
...
/**
 * Returns a random target word.
 *
 * The function avoids repeating the same word immediately
 * when a previous word is supplied and alternatives exist.
 */
const getRandomWord = (
  previousWord = ""
) => {
...
/**
 * Evaluates a submitted guess against the target word.
 *
 * This uses a two-pass algorithm so duplicate letters are
 * handled correctly.
 *
 * Example:
 *
 ...
 * Guess:  EERIE
 *
 * Exact matches are identified first. Remaining target
 * letters are then used to determine yellow matches.
 */
const evaluateGuess = (
  guess,
  target
...
/**
 * Returns the best known status for a keyboard letter.
 *
 * Priority:
 *
...
/**
 * Creates the current keyboard state from all submitted
 * guesses.
 */
...
/**
 * Converts a row of letters into a readable word.
 */
const getRowWord = (
  row
) =>
...
const Wordle = () => {
  ...
  const handleKeyboardButton =
    ...
    useEffect(() => {
      setSEO({
        title: "Wordle - DevSphere",
        description:
          "Play free Wordle on DevSphere. Guess the hidden five-letter word and solve the puzzle in limited attempts.",
        keywords:
          "Wordle, Wordle game, online Wordle, free Wordle, word puzzle, five letter word game, DevSphere Wordle",
        url: "/games/wordle",
      });
    }, []);
  ...
  return (
    <main className={styles.wordlePage}>
      ...
      >
        ...
        >
          ...
          <p
            className={
              styles.wordleSubtitle
            }
          >
            Guess the hidden five-letter
      ...
      <section
        className={
          styles.wordleSection
        }
      >

        <div
          className={
            styles.wordleContainer
          }
        >
